...
%
There are many things to be said about this, none of them are particularly edifying without a full discussion. With no apologies, I’ll make the following [perhaps perplexing] remarks, illustrated in a figure below:
\begin{enumerate}
...
\end{enumerate}
In this course these nuances will not come up. In the rest of your life you’ll still have to deal with curvilinear coordinates.\footnote{There is an excellent discussion of non-coordinate bases and curvilinear coordinates in Bernard Schutz's \emph{A First Course in General Relativity}. See Chapter 5.6 (in the first edition) ``Noncoordinate bases.''} But suffice it to say that our study of function space will be nice an orthonormal. We haven’t yet given an adequate definition of `orthonormality,’ so let's take \eqref{eq:basis:dual:vec:act:on:vec} as a working definition.
...
\begin{align}
 ...
 \begin{align}
   g_{ij}\langle i | \otimes \langle j | 
   \left( \; v^k|k\rangle
   \, , \,
   w^\ell|\ell\rangle \; \right) 
   = 
   g_ij v^k w^\ell 
 ...
 %
 For ordinary Euclidean space in flat coordinates, the metric is simply the unit matrix: $g_{ij} = \text{diag}(1,\cdots, 1)$. In Minkowksi space there’s a relative minus sign between space and time. In curvilinear coordinates things can get ugly: the value of $g_{ij}$ can depend on the position, $g_{ij} = g_{ij}(x)$.\footnote{This is related to the comment in Section~\ref{sec:lin:alg:orthonormal} that there is no such thing as `position vectors.' When we write a general metric $g_{ij}(x)$, the value of $x$ is a \emph{position} on a base space (manifold). The vectors that are `eaten' by the metric come from the \emph{tangent space at $x$}, or more generally some other vector space (fiber) at the position $x$.}
...
By the way, now is a good time to recall \eqref{eq:Ainv:acting:on:source}, and specifically our mantra: knowing the components $\left(A^{-1}\right)^i_{\phantom{i}j}$ is exactly the same as \emph{knowing the action of the inverse operator on basis vectors}. 
